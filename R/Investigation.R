...
# File:        xgapui/R/generated\java\xgap4brassica\R/Investigation.R
...
find.investigation <- function( type=NULL , id=NULL , name=NULL , start=NULL , end=NULL , .usesession = T, .verbose=T )
{
	#add session parameters
    
	result <- MOLGENIS.find( "xgap4brassica.data.types.Investigation", mget(ls(),environment()), .verbose=.verbose)
...
add.investigation <- function(.data_frame=NULL, name=NULL, start=NULL, end=NULL, .usesession = T, .verbose=T )
{
	.data_frame = .create.investigation(.data_frame, mget(ls(),environment()), .usesession = .usesession, .verbose = .verbose)
   	return( MOLGENIS.update("xgap4brassica.data.types.Investigation", .data_frame, "ADD", .verbose=.verbose) )
}
...
{	
	...
	.data_frame = .create.investigation(.data_frame, mget(ls(),environment()), .usesession = .usesession)
   	return( MOLGENIS.update("xgap4brassica.data.types.Investigation", .data_frame, "REMOVE") )
}
